...
class Brug(BrugInterface):
    ...
    def calc_intern_wegdek(self, x):
        deeltje = 2/(self.N+1)
        i = min(int(x // deeltje), len(self.Vi)) # integer deling
        N = 0
        D = sum(self.Vi[0:i]) - (C.VERDEELDE_BELASTING+C.EIG_GEWICHT_BRUG)*x - (C.PUNT_BELASTING if x > 2/3 else 0)
        ...
        for j in range(i):
            ...
            M += C.PUNT_BELASTING*(2/3 - deeltje*j) if deeltje*j < 2/3 and 2/3 < deeltje*(i+1) else 0
